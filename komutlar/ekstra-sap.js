...
exports.run = async (client, message, args) => {
    ...
    if(!roles.sap) return message.channel.send(oxyemb.setDescription(`**Ops** Sap Rolü Ayarlanmamış.`).setColor(config.renk_kırmızı)).then(o => o.delete({timeout: 10000})).catch(err => console.error(error));
    
    if(oxy.roles.cache.has(roles.sap)) {
        await oxy.roles.remove(roles.sap)  
        message.channel.send(oxyemb.setDescription(`**Başarıyla <@${oxy.id}> adlı kullanıcıdan <@&${roles.sap}> alındı.**`).setColor(config.renk_pembe))
    } else {
        await oxy.roles.add(roles.sap)
        message.channel.send(oxyemb.setDescription(`**Başarıyla <@${oxy.id}> adlı kullanıcıya <@&${roles.sap}> verildi.**`).setColor(config.renk_pembe))
    }
}
