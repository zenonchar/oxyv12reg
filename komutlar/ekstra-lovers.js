...
exports.run = async (client, message, args) => {
    ...
    if(!roles.lovers) return message.channel.send(oxyemb.setDescription(`**Ops** Lovers Rolü Ayarlanmamış.`).setColor(config.renk_kırmızı)).then(o => o.delete({timeout: 10000})).catch(err => console.error(error));
    
    if(oxy.roles.cache.has(roles.lovers)) {
        await oxy.roles.remove(roles.lovers)  
        message.channel.send(oxyemb.setDescription(`**Başarıyla <@${oxy.id}> adlı kullanıcıdan <@&${roles.lovers}> alındı.**`).setColor(config.renk_pembe))
    } else {
        await oxy.roles.add(roles.lovers)
        message.channel.send(oxyemb.setDescription(`**Başarıyla <@${oxy.id}> adlı kullanıcıya <@&${roles.lovers}> verildi.**`).setColor(config.renk_pembe))
    }
}
