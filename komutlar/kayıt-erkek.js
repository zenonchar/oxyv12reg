...
const oxyroles = require('../oxy/roles.json');
...
if(!message.member.roles.cache.has(oxyroles.registerCommander) && (!message.member.roles.cache.has(oxyroles.botCommander) && !message.member.hasPermission('VIEW_AUDIT_LOG')))
...
if(!oxyroles.erkek1) return message.channel.send(oxyemb.setDescription('**Ops** Kayıtlı Rolü Ayarlanmamış.').setColor(config.renk_kırmızı)).then(o => o.delete({ timeout: 10000 })).catch(err => console.error(error));
if(!oxyroles.unregister1) return message.channel.send(oxyemb.setDescription('**Ops** Kayıtsız Rolü Ayarlanmamış.').setColor(config.renk_kırmızı)).then(o => o.delete({ timeout: 10000 })).catch(err => console.error(error));
...
if (member.roles.cache.get(oxyroles.erkek1)) return message.channel.send(oxyemb.setDescription(`**Ops** Bu Kullanıcı <@&${oxyroles.erkek1}> Olarak Kayıtlı Durumda.`)).then(o => o.delete({ timeout: 10000 })).catch(err => console.error(error));
if (member.roles.cache.get(oxyroles.kadın1)) return message.channel.send(oxyemb.setDescription(`**Ops** Bu Kullanıcı <@&${oxyroles.kadın1}> Olarak Kayıtlı Durumda.`)).then(o => o.delete({ timeout: 10000 })).catch(err => console.error(error));
...
await oxy.roles.add(oxyroles.erkek1);
await oxy.roles.add(oxyroles.erkek2);
await oxy.roles.add(oxyroles.erkek3);
await oxy.roles.remove(oxyroles.unregister1);
await oxy.roles.remove(oxyroles.unregister2);

//=================DATA BASE=================//
db.add(`erkekKayıt_${message.author.id}`, 1)
db.add(`toplamKayıt_${message.author.id}`, 1)
db.push(`isimler_${oxy.id}`, `${oxy_isim} [**<@&${oxyroles.erkek1}>**]`);
...
message.channel.send(oxyemb.setDescription(`<@${member.id}>** adlı kullanıcı <@&${oxyroles.erkek1}> olarak kayıt edildi.** ( <#${chnl.rules}> )** Kuralları Okumayı Unutma.**\n **${liste}**`).setColor(config.renk_yesil))
